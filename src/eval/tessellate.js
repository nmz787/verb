...
verb.eval.nurbs.compute_rational_surface_max_edge_length = function( degree_u, knots_u, degree_v, knots_v, homo_control_points, tol ){
	...
	// 1) inspect the control points to determine if this is, in fact, a rational surface
	...
	// 2) if its a rational surface, modify the tolerance accoriding to Till92
	// TODO: this seems ridiculously conservative, particularly for large surfaces
	if ( Math.abs( wmax - 1.0 ) > verb.TOLERANCE || Math.abs( wmin - 1.0 ) > verb.TOLERANCE){
		...
		for (var i = 0; i < nu; i++){
			for (var j = 0; j < nv; j++){
				...
				var len = numeric.norm2( verb.eval.nurbs.dehomogenize( homo_control_points[i][j] ) );
				if (len > maxlen) maxlen = len;

			}
		}

		tol = tol * wmin / (1 + maxlen);

	}

	// 3) determine the bounds on the second derivative of the surface
	// TODO: approximate this for degree 2 or less surfaces
	...
	// the first part of this eq is to account for the diagonal edge length
	return (Math.sqrt(2) / 2) *  3 * Math.sqrt( tol / (2 * ( d2bounds[0] + d2bounds[1] + 2 * d2bounds[2])));
